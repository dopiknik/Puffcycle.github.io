$(document).ready(function () {
  // ===============================
  // INSTANT LOADER HANDLER - ПЕРВЫМ ДЕЛОМ
  // ===============================
  function hideInstantLoader() {
    const instantLoader = document.getElementById('instantLoader');
    if (instantLoader) {
      // Ждем загрузки изображений прелоадера
      const preloaderImages = document.querySelectorAll('.preloader img');
      let loadedCount = 0;
      const totalImages = preloaderImages.length;
      
      if (totalImages === 0) {
        // Если нет изображений, скрываем сразу
        instantLoader.classList.add('fade-out');
        setTimeout(() => instantLoader.style.display = 'none', 300);
      } else {
        ...
        // Проверяем, если все уже загружены
        if (loadedCount === totalImages) {
          instantLoader.classList.add('fade-out');
          setTimeout(() => instantLoader.style.display = 'none', 300);
        }
        
        // Fallback: скрываем через 3 секунды в любом случае
        setTimeout(() => {
          if (instantLoader && !instantLoader.classList.contains('fade-out')) {
            instantLoader.classList.add('fade-out');
            setTimeout(() => instantLoader.style.display = 'none', 300);
          }
        }, 3000);
  ...
  // Вызываем скрытие instant loader
  hideInstantLoader();
