...
const preloaderImages = [
  './assets/preloader/img/1.jpg',
  './assets/preloader/img/2.jpg',
  './assets/preloader/img/3.jpg',
  './assets/preloader/img/4.jpg',
  './assets/preloader/img/5.jpg',
  './assets/preloader/img/6.jpg',
  './assets/preloader/img/7.jpg',
  './assets/preloader/img/8.jpg',
  './assets/preloader/img/9.jpg',
  './assets/preloader/img/10.jpg',
  './assets/preloader/img/11.jpg',
  './assets/preloader/img/12.jpg'
];
...
// Показываем loader до загрузки картинок
window.addEventListener('DOMContentLoaded', function() {
  const loader = document.getElementById('instantLoader');
  preloadImages(preloaderImages, function() {
    // Плавно скрываем instant loader
    loader.classList.add('fade-out');
    setTimeout(function() {
      loader.style.display = 'none';
      // Если нужно, запускаем анимацию прелоадера вручную:
      // if(window.startPreloaderAnimation) window.startPreloaderAnimation();
    }, 400);
  });
});
...
$(document).ready(function () {
    ...
    fullpageInstance = new fullpage("#fullpage", {
      ...
      afterLoad: function (origin, destination) {
        ...
        if (destination.index === 3) {
          ...
          setTimeout(() => {
            if (puffcycleCurrentSlide === 1) {
              startpuffcycleAnimation();
            }
          }, 300);
        } else {
      ...
      afterSlideLoad: function (section, origin, destination) {
        ...
        if (sectionIndex === 3) {
          updatepuffcycleSlideNavigation(destination.index);
          puffcycleCurrentSlide = destination.index;

          if (destination.index === 1) {
            setTimeout(startpuffcycleAnimation, 300);
          }
        }
      },
      onLeave: function (origin, destination, direction) {
        ...
        if (sectionIndex === 3 && !isSliding) {
          if (direction === "down" && puffcycleCurrentSlide === 0) {
            isSliding = true;
            fullpage_api.moveSlideRight();
            setTimeout(() => (isSliding = false), 700);
            return false;
          }
          if (direction === "up" && puffcycleCurrentSlide === 1) {
            isSliding = true;
  ...
  function initMobileScroll() {
    ...
    $(window)
      ...
      .on("scroll.mobile", function () {
        ...
        // Trigger puffcycle animation on scroll for mobile
        if (current === "section3") {
          const heroSection = document.querySelector(
            "#section3 .puffcycle-hero-section",
          );
          if (
            heroSection &&
          ...
          const statsSection = document.querySelector(
            "#section3 .puffcycle-light-stats-section",
          );
  ...
  // ===============================
  // puffcycle FUNCTIONALITY
  // ===============================
  let puffcycleCurrentSlide = 0;
  let puffcycleIsScrolling = false;
  const puffcycleTotalSlides = 2;

  function updatepuffcycleSlideNavigation(activeIndex) {
    const navItems = document.querySelectorAll(".puffcycle-slide-nav-item");
    navItems.forEach((item, index) => {
      item.classList.toggle("active", index === activeIndex);
    });
  }

  function navigatepuffcycleSlide(direction) {
    if (isMobileMode) return;

    if (
      direction === "next" &&
      puffcycleCurrentSlide < puffcycleTotalSlides - 1
    ) {
      fullpage_api.moveSlideRight();
    } else if (direction === "prev" && puffcycleCurrentSlide > 0) {
      fullpage_api.moveSlideLeft();
    }
  }

  const puffcycleNavPrev = document.getElementById("puffcycleNavPrev");
  const puffcycleNavNext = document.getElementById("puffcycleNavNext");

  if (puffcycleNavPrev) {
    puffcycleNavPrev.addEventListener("click", () => {
      if (!isMobileMode) navigatepuffcycleSlide("prev");
    });
  }

  if (puffcycleNavNext) {
    puffcycleNavNext.addEventListener("click", () => {
      if (!isMobileMode) navigatepuffcycleSlide("next");
    });
  }

  document
    .querySelectorAll(".puffcycle-slide-nav-item")
    .forEach((item, index) => {
      item.addEventListener("click", () => {
        if (isMobileMode) return;
        if (index === 0 && puffcycleCurrentSlide !== 0) {
          fullpage_api.moveSlideLeft();
        } else if (index === 1 && puffcycleCurrentSlide !== 1) {
          fullpage_api.moveSlideRight();
        }
      });
    });

  function handlepuffcycleWheel(e) {
    if (isMobileMode) return;
    ...
    if (puffcycleIsScrolling) return;
    puffcycleIsScrolling = true;

    if (e.deltaY > 0) {
      navigatepuffcycleSlide("next");
    } else {
      navigatepuffcycleSlide("prev");
    }

    setTimeout(() => {
      puffcycleIsScrolling = false;
    }, 800);
  }

  document.addEventListener("wheel", handlepuffcycleWheel, { passive: false });

  let puffcycleTouchStartY = 0;
  let puffcycleTouchEndY = 0;
  const puffcycleMinSwipeDistance = 50;

  document.addEventListener("touchstart", function (e) {
    if (isMobileMode) return;
    const activeSection = document.querySelector(".fp-section.active");
    if (activeSection && activeSection.id === "section3") {
      puffcycleTouchStartY = e.changedTouches[0].screenY;
    }
  });
  ...
  document.addEventListener("touchend", function (e) {
    ...
    const activeSection = document.querySelector(".fp-section.active");
    if (activeSection && activeSection.id === "section3") {
      puffcycleTouchEndY = e.changedTouches[0].screenY;
      handlepuffcycleSwipe();
    }
  });

  function handlepuffcycleSwipe() {
    const swipeDistance = puffcycleTouchStartY - puffcycleTouchEndY;

    if (Math.abs(swipeDistance) > puffcycleMinSwipeDistance) {
      if (swipeDistance > 0) {
        navigatepuffcycleSlide("next");
      } else {
        navigatepuffcycleSlide("prev");
  ...
  function startpuffcycleAnimation() {
  ...
  document.addEventListener("mousemove", function (e) {
    ...
    const activeSection = document.querySelector(".fp-section.active");
    if (activeSection && activeSection.id === "section3") {
      const shapes = document.querySelectorAll(".puffcycle-shape");
  ...
  setTimeout(() => {
    const puffcycleScrollHint = document.getElementById("puffcycleScrollHint");
    if (puffcycleScrollHint) {
      puffcycleScrollHint.style.opacity = "0";
      setTimeout(() => {
        puffcycleScrollHint.style.display = "none";
      }, 500);
    }
  }, 5000);
});
