...
$(document).ready(function () {
  ...
  function highlightActiveMenuItem(sectionIndex) {
    document.querySelectorAll('.menu .nav-item, .mobile-nav-link').forEach(item => {
...
// --- Внутри initMobileScroll ---
    $('.mobile-nav-link').off('click.mobile').on('click.mobile', function(e) {
      e.preventDefault();
    ...
    $('.menu a').off('click.mobile').on('click.mobile', function(e) {
      e.preventDefault();
      const href = $(this).attr('href');
      let sectionId = '';
      switch(href) {
    ...
    let navLinks = $('.menu a, .mobile-nav-link');
    ...
    $(window).off('scroll.mobile').on('scroll.mobile', function() {
      ...
      let sectionIndex = 0;
      switch(current) {
        case 'section0': sectionIndex = 0; break;
        case 'section1': sectionIndex = 1; break;
        case 'section2': sectionIndex = 2; break;
        case 'section3': sectionIndex = 3; break;
      }
      
      highlightActiveMenuItem(sectionIndex);
      updateMenuStyle(sectionIndex);

      // Trigger CleanVape animation on scroll for mobile - улучшенная версия
      if (current === 'section3') {
        // Анимация для hero секции (первый слайд)
        const heroSection = document.querySelector('#section3 .cleanvape-hero-section');
        if (heroSection && !heroSection.classList.contains('animated-mobile')) {
          const rect = heroSection.getBoundingClientRect();
          if (rect.top < window.innerHeight * 0.8 && rect.bottom > 0) {
            heroSection.classList.add('animated-mobile');
            // Можете добавить дополнительные анимации для hero секции
          }
        }

        // Анимация для статистики (второй слайд)
        const statsSection = document.querySelector('#section3 .cleanvape-light-stats-section');
  ...
  function handleResize() {
    ...
    if (newIsMobile !== isMobileMode) {
      ...
      if (isMobileMode) {
        ...
        // Убедимся что все слайды видимы на мобильных
        setTimeout(() => {
        ...
        // Сбросить стили слайдов для desktop режима
  ...
  // Навигация по слайдам CleanVape (только для десктопа)
  function navigateCleanvapeSlide(direction) {
    if (isMobileMode) return; // Отключено на мобильных
  ...
  // Обработчики для точечной навигации CleanVape (только десктоп)
  ...
  // Обработка колесика мыши для CleanVape (только десктоп)
  ...
  // Добавляем обработчик колесика (только для десктопа)
  document.addEventListener('wheel', handleCleanvapeWheel, { passive: false });

  // Поддержка тачпада для CleanVape (только десктоп)
  ...
  // Параллакс эффект для плавающих форм CleanVape (только десктоп)
  ...
  // Поддержка клавиатуры для CleanVape (только десктоп)
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  mobileNavLinks.forEach(link => {
    link.addEventListener('click', function() {
      ...
      setTimeout(() => {
        // Navigation will be handled by mobile scroll or fullPage.js
      }, 300);
    });
  });
